...
Status ScannInterface::Initialize(std::vector<float>& dataset,
  ...
  auto dataset_ptr =
      std::make_shared<DenseDataset<float>>(std::move(dataset), n_points);
  auto status = Initialize(
      std::make_tuple(config_, dataset_ptr, std::move(opts)));
  if (recycle_dataset || !status.ok()) {
    dataset = dataset_ptr->ClearRecyclingDataVector();
  }
  return status;
}
...
int ScannInterface::Add2Index(std::vector<float>& dataset, uint32_t npoints,
                              int32_t nthreads) {
  if (static_cast<uint64_t>(npoints) * dimensionality_ != dataset.size()) {
  ...
  if (num_threads <= 0 || num_threads > GetNumCPUs()) {
    num_threads = std::max(1, GetNumCPUs() - 1);
  }
  std::shared_ptr<ThreadPool> pool = StartThreadPool("AddIndex", num_threads);
  auto* mutator = mutator_or.value();
  mutator->set_mutation_threadpool(pool);
  mutator->Reserve(n_points() + npoints);

  DenseDataset<float> add_dataset(std::move(dataset), npoints);
  for (DatapointIndex i : Seq(add_dataset.size())) {
    auto status_or_idx = mutator->AddDatapoint(add_dataset[i], "");
    if (!status_or_idx.ok()) {
      dataset = add_dataset.ClearRecyclingDataVector();
      LOG(INFO) << "AddDatapoint failed: " << status_or_idx.status().message();
      return -1;
    }
  }
  dataset = add_dataset.ClearRecyclingDataVector();

  auto maintenance_or = mutator->IncrementalMaintenance();
